  ...
  if (!opts.omitCommands) {
    base.commands = {
      run: async (command: string) => {
        ...
        return opts.commandResult ?? { exitCode: 0 };
      },
    };
  }
...
describe("type fallback diagnostics (#248)", () => {
  ...
  it("primary write fails and no clipboard utility is present (exit 127)", async () => {
    const { desktop } = makeFakeDesktop({
      write: () => {
        throw new Error("write blew up");
      },
      commandResult: { exitCode: 127, stderr: "no xclip/xsel clipboard utility available for paste fallback" },
    });
    const err = await runType(desktop);
  ...
  it("clipboard command exits nonzero with stderr: names the phase and keeps a sanitized stderr tail", async () => {
    const { desktop } = makeFakeDesktop({
      write: () => {
        throw new Error("write blew up");
      },
      commandResult: { exitCode: 1, stderr: "clipboard utility present but failed to set the clipboard" },
    });
    const err = await runType(desktop);
    expect(err).toBeInstanceOf(CuaTypeFallbackError);
