...
`mimetic/scenarios/*.yaml`; no builtin fallback on the lab route — the steps ARE the
actor). `mimetic run --app-url` behavior stays byte-identical.

Ratified trace-vocabulary additions to `mimetic.actor-trace.v1` (additive, conformance
...
- `execution.concurrency` promoted to consumed on the cua route: default
  `min(laneCount, 3)`; env `MIMETIC_CUA_MAX_CONCURRENCY` may only LOWER the effective
...
- per-lane xdpyinfo geometry assertion (fail-closed `MIMETIC_CUA_LAB_DEVICE_GEOMETRY`)
...
- evidence: run bundle stays `mimetic.run-bundle.v1` (N streams; verify/Observer
  untouched — verified N-ary already); result bumps honestly to
  `mimetic.cua-lab-result.v2` with `lanes[]` ALWAYS present (length 1 at N=1; N=1
...
→ teardown kills all + always deletes the snapshot (deterministic `mimetic-<runId>`
...
`MIMETIC_MAX_LANES` escape above 16 until a reference panel demands it.
...
(`MIMETIC_LIVE_CUA=1`), run manually, asserting ENGAGEMENT (actions+messages>0, read
